...
# Return 'yes' only when the model has completed ALL epochs in the config.
# Checks metrics.csv row count — not finished_at, which is also written on
# Ctrl+C and would wrongly skip an interrupted model.
is_finished() {
    python -c "
...
CONFIGS=(
    ...
    CBC_config/config_densenet_ZS_50k.yml
    CBC_config/config_densenet_ZS_100k.yml

    CBC_config/config_DBC_10k.yml
    CBC_config/config_DBC_100k.yml

    CBC_config/config_DBC_ZS_10k.yml
    CBC_config/config_DBC_ZS_50k.yml
    CBC_config/config_DBC_ZS_100k.yml
)
# CONFIGS=(
#     # Custom CNN (CBC)
